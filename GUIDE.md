...
- `gc_stats()` - структура `{allocated, limit, bytesEstimate}` з обліком NyxilumLang-виділень (масиви/структури/мапи) поточного запуску; `gc_collect()` - форсує збирання сміття .NET і оновлює оцінку пам'яті; `gc_limit(n)` - встановлює ліміт кількості виділень, перевищення кидає помилку (можна зловити через `try/catch`, або задати ззовні через `NX_GC_MAX_OBJECTS`). НЕ захищає від нескінченної рекурсії без виділень (масивів/структур/мап) - для цього окремий ліміт глибини викликів нижче.
- Глибина викликів обмежена 10 000 (func, метод структури, значення-функція) - функція без умови виходу (напр. `func f(n) { return f(n+1) }`) кидає `try/catch`-зловлювану помилку замість необмеженого росту пам'яті аж до OOM
- Вкладеність виразів/блоків і довжина ланцюжків операторів обмежені (500) - надто глибокі дужки/виклики/блоки `if`/довгі ланцюжки `+`,`!`,`=` тощо кидають Runtime Error ще на етапі парсингу файлу, а не native stack overflow (той не спіймати жодним try/catch, ні в NyxilumLang, ні в .NET)
...
- `osDiskFree(path?)` - `{freeGB, totalGB}` для тому, що містить `path` (типово поточна директорія)
- `notify(title, message)` - системне push-сповіщення ОС (не власне вікно програми - справжній баблик поверх усіх вікон, як у месенджерів): `notify-send` на Linux, `osascript` на macOS, `NotifyIcon`-балон через PowerShell на Windows
...
- `wsConnect(url)` - відкриває WebSocket-з'єднання (`wss://`/`ws://`); `wsSend(ws, text)` - надсилає текстове повідомлення; `wsReceive(ws, timeoutMs?)` - блокує до наступного повідомлення або тайм-ауту (тоді повертає `null`, і саме з'єднання лишається цілком робочим - можна викликати wsSend/wsReceive/wsClose далі як звичайно); кидає помилку (лови через `try/catch`), якщо з'єднання розірване сервером; `wsClose(ws)` - закриває з'єднання (тайм-аут 5с на очікування close-фрейму у відповідь - якщо інший бік не відповідає, сокет однаково закривається локально, а не блокує скрипт назавжди)
